...
def home(request):
    ...
    # webcount = WebCount.objects.all().first()
    # print(webcount.count)
...
def pdfview(request):
    if request.method == 'POST':
        ...
        return render(request, 'pdfview.html',{'pdf_data_base64': pdf_data_base64,"filename":pdffile})
...
def pdfdownload(request):
    if request.method == 'POST':
        ...
        responses = HttpResponse(BytesIO(bytesdata),content_type='application/octet-stream')
